...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def reset(self) -> Observations:
        ...
        return self._sensor_suite.get_observations(sim_obs)#, dataset=self.habitat_config.SCENE.split("/")[-2])

    def step(self, action) -> Observations:
        sim_obs = super().step(action)
        #print(time.time() - s)
        self._prev_sim_obs = sim_obs
        observations = self._sensor_suite.get_observations(sim_obs)#, dataset=self.habitat_config.SCENE.split("/")[-2])
        return observations
